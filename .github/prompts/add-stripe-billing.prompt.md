...
## PaidSoon Context
Stripe billing uses `stripe@22.1.1` with API version `"2026-05-27.dahlia"`. Four public tiers: `essentials`, `business_control`, `small_business`, and `business_pro`, plus hidden `accountant_partner` (contact-us pricing — planned, not yet fully implemented; no Stripe Checkout for this tier). Checkout, customer portal, and billing webhooks are all implemented.
