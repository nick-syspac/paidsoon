...
- [stripe.md](./stripe.md) §1–§4 — API keys and billing price IDs (`STRIPE_STARTER_PRICE_ID`, `STRIPE_SOLO_PRICE_ID`, `STRIPE_SMALL_BUSINESS_PRICE_ID`, `STRIPE_BUSINESS_PRO_PRICE_ID`) exist in both modes; `STRIPE_CONNECT_CLIENT_ID` exists at least in test mode (live mode requires Connect approval).
...
| `STRIPE_BUSINESS_PRO_PRICE_ID` | ✓ (live `price_…`) | ✓ (test `price_…`) | ✓ (test `price_…`) |
