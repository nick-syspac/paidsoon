...
| Weekly debtor summary | `worker/paidsoon_worker/` job + `app/api/internal/jobs/send-weekly-debtor-summary` | Non-functional in production | Only invoked by the undeployed Railway worker; no Vercel Cron equivalent exists. Customer-facing "available/live" claims must remain disabled until restore criteria are met: (1) active production scheduler path, and (2) successful delivery evidence for at least one full weekly cycle. Reactivation is linked to `openspec/changes/migrate-scheduled-jobs-to-railway-celery` tasks 8.1-8.3 (parity -> cutover -> cron retirement). |
