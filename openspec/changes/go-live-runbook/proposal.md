...
- **Stripe**: Apply for Stripe Connect platform access; create the four public billing prices (Essentials/Business Control/Small Business/Business Pro); register billing and Connect webhooks pointing at the production deployment.
...
- **Secrets created/rotated**: `SUPABASE_PROJECT_REF`, `SUPABASE_DB_PASSWORD`, `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`, `SUPABASE_SECRET_KEY`, `STRIPE_SECRET_KEY`, `STRIPE_STARTER_PRICE_ID`, `STRIPE_SOLO_PRICE_ID`, `STRIPE_SMALL_BUSINESS_PRICE_ID`, `STRIPE_BUSINESS_PRO_PRICE_ID`, `STRIPE_CONNECT_CLIENT_ID`, `STRIPE_BILLING_WEBHOOK_SECRET`, `STRIPE_CONNECT_WEBHOOK_SECRET`, `RESEND_API_KEY`, `RESEND_FROM_EMAIL`, `RESEND_FROM_NAME`, `CRON_SECRET`, `NEXT_PUBLIC_APP_URL`
