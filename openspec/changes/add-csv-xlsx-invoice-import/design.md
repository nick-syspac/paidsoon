...
- Upload-time staging capture: the upload step parses the file once and writes one `InvoiceImportStagingRow` per row with the full set of detected source columns (`raw`), not just canonical-recognized ones. Later mapping-submit and validation calls read from these staging rows instead of re-parsing the original file, so the mapping step has a stable data contract independent of file retention.
...
- No new customer/invoice schema: PaidSoon has no standalone customer table — `TrackedInvoice` denormalizes `clientEmail`/`clientName` directly and already anticipates non-Stripe sources via its `provider` field. Imports reuse this model with a dedicated `provider = "spreadsheet_import"` tag and a shadow `InvoiceConnection` row (found-or-created per user, same pattern the accounting-sync orchestrator uses for Xero/MYOB) to satisfy the required FK, instead of adding a parallel customer/invoice schema. Import-only fields (`customer_contact_name`, `purchase_order_reference`, `payment_url`, `invoice_description`, `notes`, `customer_external_id`, `invoice_external_id`, `invoice_total`, `invoice_date`) are stored in the existing `providerMetadata` JSON column, matching how other providers attach source-specific data.
- Invoice matching key: the identifier used to look up an existing `TrackedInvoice` (and to detect within-batch duplicates) is `invoice_external_id` when present, otherwise `invoice_number`, scoped by `(externalId, provider, userId)` — the same idempotency key every other invoice source uses. Customer matching is by `customer_email` (the only customer identity `TrackedInvoice` has); `customer_external_id` is carried as metadata and cross-checked within a batch so the same external ID can't silently resolve to two different email addresses.
