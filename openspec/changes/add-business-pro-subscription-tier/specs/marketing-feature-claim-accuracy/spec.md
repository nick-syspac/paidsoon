...
Marketing copy that attributes a feature to a specific subscription tier SHALL name a tier that currently exists in the plan catalog (`essentials`, `business_control`, `small_business`, `business_pro`, or `accountant_partner`) and SHALL match the tier at which `lib/subscriptionPlans.ts` actually enables that feature.

#### Scenario: AI rewrite is attributed to the correct tier
- **WHEN** a marketing page describes AI-assisted reminder wording
- **THEN** it attributes the feature to Business Control and above, not to a retired "Business" tier name
...
- **THEN** it distinguishes custom sender name (Business Control and above) from a verified custom sending domain (Small Business and above) rather than presenting both as available on the same set of tiers
