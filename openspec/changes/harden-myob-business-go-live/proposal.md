...
**Resolved by `fix-myob-company-file-identity`:** production callback logs confirmed `businessId` (and an undocumented `businessName`) are returned directly on the callback query string, and MYOB's own docs confirm one company file is authorised per OAuth grant. That change removed the `getOrganisations()`/`select-org` picker entirely and resolves the company file directly from the callback — see that change's proposal/design for the full root-cause analysis and fix.