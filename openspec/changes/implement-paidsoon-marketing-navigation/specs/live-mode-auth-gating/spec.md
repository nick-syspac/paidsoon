...
### Requirement: Public marketing routes SHALL remain accessible in not-live mode
All public marketing routes (including `/`, `/features`, `/pricing`, `/how-it-works`, `/accountants`, `/resources`, `/contact`, `/about`, `/careers`, `/integrations`, `/roadmap`, `/blog`, `/help`, `/docs`, `/faq`, `/release-notes`, `/privacy`, `/terms`, `/cookies`, `/security`, `/acceptable-use`) SHALL return HTTP 200 and render their content regardless of `LIVE` mode. The live-mode gate SHALL NOT block these routes.

#### Scenario: Marketing routes accessible when LIVE=false
- **WHEN** `LIVE=false` and a visitor navigates to `/features`
- **THEN** the features page renders with HTTP 200
- **THEN** the visitor is NOT redirected to a blocked or unavailable view

#### Scenario: Marketing routes accessible when LIVE=true
- **WHEN** `LIVE=true` and a visitor navigates to `/contact`
- **THEN** the contact page renders with HTTP 200
