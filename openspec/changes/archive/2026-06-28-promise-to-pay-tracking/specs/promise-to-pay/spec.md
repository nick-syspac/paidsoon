...
- **THEN** the system displays the invoice reference (client name, amount due, currency) and a form to enter a commitment date, optional amount, and optional note
...
- **WHEN** a client submits the promise form with a future date
...
- **THEN** the invoice has no remaining `active` promise and subsequent cron runs will send scheduled emails normally

---

### Requirement: Promise breach detection and freelancer notification

The system SHALL detect broken promises during the daily cron run. A promise is broken when its `promisedPayBy` date has passed and the invoice is not in `paid` or `manually_resolved` status. On detection, the system SHALL mark the promise `status: 'broken'`, record `breachNotifiedAt`, and send a breach notification email to the freelancer.
...
- **THEN** the email SHALL include the client name, the promised amount or full invoice amount, the promise date that was missed, the number of prior broken promises by that client email, and a link to the dashboard
...
- **THEN** the freelancer receives an email containing the client name, committed payment date, optional amount, optional client note, and a link to the dashboard invoice

---

### Requirement: Dashboard promise indicators

The system SHALL display promise state on the invoice dashboard table for invoices that have an active or broken promise.
...
- **THEN** the dashboard invoice row displays a broken promise warning including how many prior broken promises exist for that client email
