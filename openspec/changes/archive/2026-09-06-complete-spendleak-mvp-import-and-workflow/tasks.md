...
## 2. Owner Review Workflow (MVP Actions, Canonical-Layer Compatible)

- [x] 2.1 Extend SpendLeak finding data model and API contract for owner actions: keep, cancel, renegotiate, ignore, and reopen, while preserving canonical financial record references in evidence payloads.
...
## 3. Dashboard and Summary Integration Over Canonical Inputs

- [x] 3.1 Surface findings originating from expense imports alongside provider-sync findings in SpendLeak modules and lists, using the shared canonical provenance vocabulary (`sourceSystem`/`sourceId`).
