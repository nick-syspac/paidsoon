...
- [ ] 2.4 Update finding detail and related SpendLeak UI surfaces to show/apply decision actions and current review outcome.

## 3. Dashboard and Summary Integration

- [ ] 3.1 Surface findings originating from expense imports alongside provider-sync findings in SpendLeak modules and lists.
- [ ] 3.2 Include decision outcomes in SpendLeak overview/detail presentation so reviewed status is visible without opening raw evidence.
- [ ] 3.3 Ensure grounded summaries remain evidence-based when findings come from mixed sources (provider + imports).

## 4. Verification and Release Readiness

- [ ] 4.1 Add tests for CSV/XLSX expense import parsing, mapping, validation errors, and idempotent commit.
- [ ] 4.2 Add tests for owner action transitions, suppression behavior, and cross-tenant denial.
- [ ] 4.3 Add/update integration tests covering dashboard visibility for import-sourced findings and decision outcomes.
- [ ] 4.4 Run `openspec validate complete-spendleak-mvp-import-and-workflow --strict` and record readiness for `/opsx:apply`.