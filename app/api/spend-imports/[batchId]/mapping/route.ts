...
import { getMissingRequiredSpendMappingFields } from "@/lib/spendImport/mapping"
import { isSpendImportCanonicalField, type SpendImportCanonicalField } from "@/lib/spendImport/template"
...
const MappingRequestSchema = z.object({
  ...
  duplicateMode: z.enum(["skip_existing", "update_existing"]).optional(),
})
...
export async function POST(request: Request, { params }: Params): Promise<NextResponse> {
  ...
  const invalidField = Object.values(parsed.data.mapping).find((field) => !isSpendImportCanonicalField(field))
  if (invalidField) {
    return NextResponse.json({ error: `Unknown target field: ${invalidField}` }, { status: 400 })
  }

  const mapping = parsed.data.mapping as Record<string, SpendImportCanonicalField>
  ...
  const result = await withUserContext(user.id, async (tx) => {
    ...
    await tx.spendImportColumnMapping.deleteMany({ where: { batchId } })
    await tx.spendImportColumnMapping.createMany({
